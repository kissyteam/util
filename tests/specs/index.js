...
describe('util', function () {
    ...
    describe('util.mix', function () {
        it('allow non structured deep mix', function () {
            ...
            (function () {
                ...
                expect(n).to.deep.equal(a);
                expect(n.arr[0]).not.to.equal(n.arr[1]);
                expect(n.arr[0]).to.deep.equal(n.arr[1]);
            })();
        ...
        it('solve JsEnumBug', function () {
            ...
            fs.sort();
            expect(fs).to.deep.equal(['toString',
                'hasOwnProperty', 'isPrototypeOf',
                'propertyIsEnumerable',
                'toLocaleString',
                'valueOf',
                'constructor'].sort());
            expect(vs).to.deep.equal([1, 1, 1, 1, 1, 1, 1]);
        });
    ...
    describe('util.clone', function () {
        it('works', function () {
            ...
            var date = new Date();
            expect(util.clone(date)).to.deep.equal(date);
            expect(util.clone(date)).not.to.equal(date);


            var reg = /i/i;
            expect(util.clone(reg)).to.deep.equal(reg);
            ...
            expect(Tom2.son).to.deep.equal(Green);
            ...
            expect(Green2.father).to.deep.equal(Tom);
            ...
            expect(t10[0].z).to.deep.equal(t7);

            // 复制后仍是同一数组
            expect(t10[1].q).to.equal(t10[0].q);
            expect(t10[1].q).not.to.equal(t22);
            expect(t10[1].q).to.deep.equal(t22);
    ...
    it('util.map', function () {
        ...
        expect(plurals).to.deep.equal(["feet", "geese", "meese"]);
    ...
    it('util.keys', function () {
        ...
        expect(ret).to.deep.equal(['x', "toString"].sort());
    });
